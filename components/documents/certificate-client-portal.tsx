...
export default function CertificatePageClient({ userRole }: CertificatePageClientProps) {
  ...
  useEffect(() => {
    const initData = async () => {
      ...
      if (stdsRes.success) {
        setStudents(stdsRes.data)
        if (isStudent && stdsRes.data.length > 0) {
          setSelectedStudent(stdsRes.data[0])
          setStep("preview")
        }
      }
  ...
  return (
    <>
      <DashboardHeader 
        title={isStudent ? "Mon Certificat de Scolarité" : "Certificat de Scolarité"} 
        subtitle={isStudent 
          ? "Consultez, imprimez ou téléchargez votre certificat officiel"
          : "Générez instantanément des certificats officiels sécurisés"
        }
      />
        ...
        {step === "select" && !isStudent ? (
          ...
          <div className="grid gap-8 lg:grid-cols-3 items-start">
            <div className="lg:col-span-2 space-y-4">
              ...
              {/* High-Fidelity On-Screen Certificate Preview */}
              {selectedStudent ? (
                <CertificateTemplate 
                  student={selectedStudent} 
                  schoolInfo={schoolInfo} 
