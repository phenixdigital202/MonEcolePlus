                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 font-medium mt-1">{schoolInfo.adresse} {schoolInfo.telephone && `• ${schoolInfo.telephone}`}</p>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex justify-between items-center pb-4 border-b border-slate-200">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">{schoolInfo.adresse}</p>
                  </div>
                </div>
                <div className="text-right">
...
export default function BulletinClientPage({ userRole, currentUser, initialClassId }: BulletinClientPageProps) {
  ...
  useEffect(() => {
    const initData = async () => {
      ...
      setClasses(clsList)
      let targetClassId = initialClassId
      if (initialClassId && clsList.some((c: any) => c.id.toString() === initialClassId)) {
        setSelectedClass(initialClassId)
      } else if (clsList.length > 0) {
        targetClassId = clsList[0].id.toString()
      ...
      if (isStudent && targetClassId) {
        ...
        const res = await getBulletinFullClassDataAction(parseInt(targetClassId), selectedSemester)
        if (res.success && res.data) {
          setReportData(res.data)
          if (currentUser?.id) {
            const myIndex = res.data.students.findIndex((s: any) => s.id === currentUser.id)
            setSelectedStudentIndex(myIndex !== -1 ? myIndex : 0)
          } else {
    ...
    initData()
  }, [initialClassId, isStudent, currentUser?.id, selectedSemester])
  ...
  // Filter students list for sidebar if student role
  const displayStudentsList = isStudent && currentUser?.id 
    ? (reportData?.students || []).filter((s: any) => s.id === currentUser.id)
    : (reportData?.students || [])

  return (
    <>
      <DashboardHeader 
        title={isStudent ? "Mon Bulletin Scolaire" : "Bulletin de Notes SaaS"} 
        subtitle={isStudent 
          ? "Consultez et téléchargez vos bulletins officiels"
          : "Générez des bulletins premium calculés en direct sur la base de données"
        }
      />
      
      <main className="p-4 md:p-8 max-w-6xl mx-auto">
        {step === "select" && !isStudent ? (
                ...
                <div className="flex flex-col sm:flex-row gap-3 justify-between items-center bg-slate-100/70 p-3 rounded-2xl border border-slate-200 print:hidden">
                   {!isStudent ? (
                     ...
                     <div className="flex items-center gap-2 text-xs font-bold text-slate-700">
                        <span>Période :</span>
                        <Select value={selectedSemester} onValueChange={setSelectedSemester}>
                           <SelectTrigger className="h-9 border-slate-200 rounded-xl text-xs bg-white w-32 font-bold">
                              <SelectValue />
                           </SelectTrigger>
                  ...
                  <div className="p-16 text-center text-slate-400 italic bg-white rounded-3xl border">
                    {isCalculating ? (
                      <div className="flex flex-col items-center justify-center gap-2">
                        ...
                        <p className="text-sm text-slate-600 font-medium">Chargement de votre bulletin...</p>
                      </div>
                    ) : (
                ...
                <Card className="border-slate-200 bg-white rounded-3xl shadow-lg overflow-hidden">
                   <CardHeader className="bg-slate-50 border-b">
                      <CardTitle className="text-sm font-bold uppercase text-slate-600">
                        {isStudent ? "Fiche de l'Élève" : `Élèves de la Classe (${reportData?.students?.length || 0})`}
                      </CardTitle>
                   </CardHeader>
                   <CardContent className="p-3 space-y-1.5 max-h-[500px] overflow-y-auto custom-scrollbar">
                      {displayStudentsList.map((s: any, idx: number) => {
                        const actualIdx = (reportData?.students || []).findIndex((st: any) => st.id === s.id)
                        const isSelected = actualIdx === selectedStudentIndex
                        return (
                          <div 
                            key={s.id} 
                            className={`flex items-center justify-between p-3 rounded-2xl cursor-pointer transition-all ${isSelected ? 'bg-primary text-white shadow-md' : 'hover:bg-slate-50 border border-slate-100'}`}
                            onClick={() => setSelectedStudentIndex(actualIdx !== -1 ? actualIdx : 0)}
                          >
                             <div className="flex items-center gap-2.5">
                                <div className={`h-7 w-7 rounded-xl flex items-center justify-center font-bold text-xs ${isSelected ? 'bg-white/20 text-white' : 'bg-primary/10 text-primary'}`}>
                                   {s.rank}
                                </div>
                                <div>
                                   <p className="text-xs font-bold leading-tight">{s.nom}</p>
                                   <p className={`text-[10px] ${isSelected ? 'text-white/80' : 'text-slate-500'}`}>Moy: {s.overallAvg.toFixed(2)}/20</p>
                                </div>
                             </div>
